...
export const fetchDelete = async (url: string): Promise<FetchResponse> => {
  ...
  await axios
    .delete(`${import.meta.env.VITE_BASE_URL_API}${url}`, {
      headers: token
        ? {
            accept: "application/json",
            Authorization: `Bearer ${token}`,
...
export const fetchPost = async (
  ...
  let data = null;
  let error = null;
  const token = forcedToken ? forcedToken : store.getState().auth.token;

  await axios
    .post(`${import.meta.env.VITE_BASE_URL_API}${url}`, payload, {
      headers: token
        ? {
            accept: "application/json",
            Authorization: `Bearer ${token}`,
...
export const fetchPut = async (
  ...
  await axios
    .put(`${import.meta.env.VITE_BASE_URL_API}${url}`, payload, {
      headers: token
        ? {
            accept: "application/json",
            Authorization: `Bearer ${token}`,
...
export const fetchGet = async (url: string, forcedToken: string = ""): Promise<FetchResponse> => {
  let data = null;
  let error = null;
